...
from nornir.core.exceptions import NornirExecutionError, NornirSubTaskError, AsyncError
...
CoroutineFunction = Callable[[Any, Any], Awaitable[Any]]
...
class AsyncTask:
    ...
    async def run(self, task: CoroutineFunction, **kwargs: Any) -> "MultiResult":
        ...
        This is a utility method to call a task from within a task. For instance:
            ...
            async def grouped_tasks(task):
                await task.run(my_first_task)
                await task.run(my_second_task)

            await nornir.async_run(grouped_tasks)
